  ...
  <q-scroll-area class="full-height full-width">
    <div class="q-pa-lg">
      <div class="row q-mb-md">
        <div class="col text-h5">{{ $t('CORELITEUSERS.HEADING_SETTINGS_TAB') }}</div>
      </div>
      <q-card flat bordered class="card-edit-settings">
        <q-card-section>
          <div class="row">
            <q-checkbox dense v-model="isLiteUser">
              <q-item-label v-t="'CORELITEUSERS.LABEL_IS_LITE_USER'"/>
            </q-checkbox>
          </div>
        </q-card-section>
      </q-card>
...
export default {
  name: 'CoreLiteUsersAdminSettingsPerUser',
  ...
  methods: {
    getSettingsFromUser () {
      ...
      return typesUtils.pBool(userCompleteData['CoreLiteUsers::IsLite'])
    },
     ...
     */
    hasChanges () {
      const isLiteUser = this.getSettingsFromUser()
      return this.isLiteUser !== isLiteUser
    },
     ...
     */
    revertChanges () {
      this.isLiteUser = this.getSettingsFromUser()
    },
    ...
    updateSettingsForEntity () {
      if (!this.saving) {
        ...
        const parameters = {
          ...
          IsLiteUser: typesUtils.pBool(this.isLiteUser),
        }
        webApi.sendRequest({
          ...
          if (result) {
            cache.getUser(parameters.TenantId, this.user?.id).then(({ user }) => {
              user.updateData([{
                field: 'CoreLiteUsers::IsLite',
                value: typesUtils.pBool(this.isLiteUser)
              }])
              this.populate()
            })
